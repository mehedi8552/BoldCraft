...
export default function HomeService() {
  return (
    <div className=" bg-[#F9FAFB] dark:bg-[#1f1f25]  bg-cover bg-center bg-no-repeat">
      <Container className="py-32 flex flex-col md:flex-row justify-between ">
        <div className="mt-[30px]">
          <h1 className="text-section-title font-bold dark:text-white">Services</h1>
          <p className="Mulish-regular mt-4 mr-4">
          ...
          {ServiceData.slice(0, 4).map((data) => {
            return (
              <motion.div
                ...
                className="cursor-pointer hover:shadow-lg md:p-4 py-8 rounded-md space-x-2 space-y-4 gradient-hover group border dark:border-gray-200"
              >
                <span className="text-6xl group-hover:text-white text-red-500">
                  {data.icons}
                </span>
                <h1 className="Mulish-medium  text-card-title group-hover:text-white dark:text-white">
                  {data.name}
                </h1>
                <p className="Mulish-regular text-fontPrimary text-gray-600 group-hover:text-white dark:text-gray-400">
                  {data.description}
                </p>
              </motion.div>
            );
